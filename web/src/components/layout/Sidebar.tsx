...
// Rail footer — the version tag, centered, with a native tooltip carrying the
// full "ZeroClaw Gateway vX" string since the rail has no room for the label.
function RailFooter({ version }: FooterProps) {
  const title = version ? `${t('sidebar.gateway')} v${version}` : t('sidebar.gateway');
  return (
    <div
    ...
    >
      {version && (
        <span title={title} style={{ fontSize: '9px', color: 'var(--pc-text-faint)' }}>
          v{version}
        </span>
      )}
    </div>
...
function DrawerFooter({ version }: FooterProps) {
  return (
    <div
