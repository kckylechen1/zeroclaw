...
fn tool_allowed_in_context(name: &str, exclude_memory: bool) -> bool {
...
impl ScopedToolRegistry {
    ...
    pub async fn assemble(spec: ScopedAssembly<'_>) -> ScopedAssembled {
        ...
        //    recover memory tools removed from the outer registry.
        tools_registry.retain(|tool| tool_allowed_in_context(tool.name(), exclude_memory));
...
mod tests {
    ...
    async fn sequential_pipeline_prevalidates_memory_excluded_by_assembly_context() {
        assert_pipeline_context_prevalidates_excluded_tool("memory_recall", true, false).await;
    }

    #[tokio::test]
    async fn parallel_pipeline_prevalidates_memory_excluded_by_assembly_context() {
        assert_pipeline_context_prevalidates_excluded_tool("memory_recall", true, true).await;
    }
