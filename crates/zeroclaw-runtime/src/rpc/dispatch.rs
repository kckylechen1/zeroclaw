...
pub(crate) type BoxRpcFuture<'a> =
    std::pin::Pin<Box<dyn std::future::Future<Output = RpcResult> + Send + 'a>>;
...
pub(crate) fn session_should_initialize_mcp(chat_mode: &crate::rpc::types::ChatMode) -> bool {
...
pub struct RpcDispatcher {
    ...
    pub(crate) client_elicitation_caps: zeroclaw_api::elicitation::ElicitationCapabilities,
}
...
impl RpcDispatcher {
    ...
    pub(crate) async fn forward_seed_event(&self, session_id: &str, event: Option<TurnEvent>) {
    ...
    pub(crate) async fn flush_config(&self, _guard: &ConfigWriteGuard) -> Result<(), JsonRpcError> {
        debug_assert!(
    ...
    /// flight.
    pub(crate) async fn save_and_swap_config(
        &self,
    ...
    pub(crate) async fn agent_rename_residue_exists(
        &self,
        config: &zeroclaw_config::schema::Config,
        from: &str,
    ) -> bool {
    ...
    pub(crate) async fn process_line(&mut self, line: &str) {
    ...
    pub(crate) fn schedule_daemon_reload(&self, surface: &'static str) -> bool {
...
pub(crate) fn validate_session_configure_overrides(
...
pub(crate) async fn persist_acp_turn(
...
pub(crate) fn plan_replay_notification(
    session_id: &str,
