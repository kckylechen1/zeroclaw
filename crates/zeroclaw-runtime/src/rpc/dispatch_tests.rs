...
fn plan_turn_event_maps_to_plan_notification() {
    ...
    let json =
        notification_for_turn_event("sess-1", &event, None).expect("plan yields a notification");
    let v = parse(&json);
...
async fn session_messages_falls_back_to_acp_store_for_acp_sessions() {
    ...
    let parsed: SessionMessagesResult = from_value(result).expect("SessionMessagesResult shape");
...
async fn wait_for_temperature(dispatcher: &RpcDispatcher, session_id: &str, expected: Option<f64>) {
    for _ in 0..50 {
...
async fn session_cancel_from_owner_dispatcher_still_works() {
    let tmp = tempfile::TempDir::new().unwrap();
    let config = make_acp_test_config(&tmp);
    let (mut dispatcher_a, _dispatcher_b, sessions) = make_two_dispatchers_sharing_context(config);
...
async fn alias_rename_serializes_with_config_set() {
    ...
    let (tx, _rx) = tokio::sync::mpsc::channel(64);
    let mut rename_dispatcher = RpcDispatcher::new(Arc::clone(&ctx), tx, "test-peer-rename".into());
