//! Outbound response sanitization: tool-protocol stripping, think-tag
//! removal, and credential leak redaction for text that leaves the process
//! for a person (a channel, a gateway client, or a bridge).
//!
//! Moved here from `zeroclaw-channels` (`orchestrator/outbound_sanitize.rs`)
//! so the channels, the gateway `/ws/chat` stream, and the bridge outbox share
//! one implementation. [`OutboundStreamRedactor`] applies the same passes to a
//! streamed reply, holding back any tail that could still turn into a secret.
...
use super::{LeakDetector, LeakResult};
...
pub fn strip_tool_result_content(text: &str) -> String {
...
pub fn strip_tool_summary_prefix(text: &str) -> String {
...
pub fn strip_think_tags_inline(s: &str) -> String {
...
pub fn sanitize_streaming_draft_text(s: &str, known_tool_names: &HashSet<String>) -> String {
...
pub enum OutboundContentFormat {
    Markdown,
    PlainText,
}

pub fn outbound_content_format_for_channel(channel: &str) -> OutboundContentFormat {
...
pub fn sanitize_channel_response_for_format_with_leak_detection(
    response: &str,
    tools: &[Box<dyn Tool>],
    ...
    sanitize_outbound_response(
        response,
        &known_tool_names(tools.iter().map(|tool| tool.name())),
        leak_detection,
        content_format,
    )
}

/// The lowercase tool-name inventory the protocol passes judge payloads by.
pub fn known_tool_names<'a>(names: impl IntoIterator<Item = &'a str>) -> HashSet<String> {
    names.into_iter().map(str::to_ascii_lowercase).collect()
}

/// Final outbound sanitization for a complete reply: think tags, tool-call
/// and tool-result envelopes, protocol JSON, tool narration, then credential
/// leak redaction. `known_tool_names` is the lowercase tool inventory (see
/// [`known_tool_names`]).
pub fn sanitize_outbound_response(
    response: &str,
    known_tool_names: &HashSet<String>,
    leak_detection: &LeakDetectionConfig,
    content_format: OutboundContentFormat,
    ...
    let stripped_json = strip_isolated_tool_json_artifacts(&stripped_fenced_json, known_tool_names);
...
pub fn redact_channel_outbound_leaks(
    content: &str,
    leak_detection: &LeakDetectionConfig,
    content_format: OutboundContentFormat,
) -> String {
    ...
    match scan_outbound_leaks(content, leak_detection, content_format) {
...
/// The leak scan behind [`redact_channel_outbound_leaks`], without its log
/// record, for callers that rescan the same growing text (the stream
/// redactor) and would otherwise log one warning per chunk.
fn scan_outbound_leaks(
    content: &str,
    leak_detection: &LeakDetectionConfig,
    content_format: OutboundContentFormat,
) -> LeakResult {
    ...
    LeakDetector::with_config(leak_detection).scan_with_protected_spans(content, &protected_spans)
}

pub fn channel_outbound_protected_spans(
    content: &str,
    content_format: OutboundContentFormat,
) -> Vec<Range<usize>> {
...
/// Sanitizes a reply that reaches its client as appended chunks, where text
/// already sent cannot be taken back.
///
/// Each push re-runs the streaming draft passes ([`sanitize_streaming_draft_text`])
/// and leak redaction over everything received so far, then releases only
/// text that nothing still arriving can change:
///
/// - the trailing whitespace-delimited token is held back, so a credential
///   split across chunks (`sk-abc` + `def...`) is judged whole before any of
///   it is sent;
/// - text from an unclosed `-----BEGIN` marker is held back until its
///   `-----END` arrives and the whole block can be redacted.
///
/// When a later pass rewrites text that was already released (a redaction
/// that starts inside it, or an envelope that closes around it), the stream
/// stops for the rest of the turn instead of emitting a continuation that no
/// longer lines up. The caller's final frame carries the complete reply
/// through [`sanitize_outbound_response`], so the client ends on the same
/// text a channel would deliver.
///
/// The held-back tail is at most the last token plus an open PEM block; the
/// accumulated text is the turn's reply, which the caller keeps anyway.
pub struct OutboundStreamRedactor {
    received: String,
    released: String,
    halted: bool,
    known_tool_names: HashSet<String>,
...
impl OutboundStreamRedactor {
    pub fn new(
        ...
        Self {
            ...
            halted: false,
            known_tool_names,
            leak_detection: leak_detection.clone(),
            content_format,
        }
    }

    /// Add a raw delta and return the next piece of sanitized text that is
    /// safe to send, if any.
    pub fn push(&mut self, delta: &str) -> Option<String> {
        ...
        let view = sanitize_streaming_draft_text(&self.received, &self.known_tool_names);
        let view = if self.leak_detection.enabled {
            match scan_outbound_leaks(&view, &self.leak_detection, self.content_format) {
                LeakResult::Clean => view,
                LeakResult::Detected { redacted, .. } => redacted,
            }
        } else {
            view
        };
        if !view.starts_with(self.released.as_str()) {
            if self.released.starts_with(view.as_str()) {
                // Shorter for now (an opener being held back); wait for more.
                return None;
            }
            self.halted = true;
            ::zeroclaw_log::record!(
                DEBUG,
                ::zeroclaw_log::Event::new(module_path!(), ::zeroclaw_log::Action::Note)
                    .with_attrs(::serde_json::json!({"released_len": self.released.len()})),
                "outbound stream rewritten behind released text; holding the rest for the final frame"
            );
            return None;
        }
        let end = stable_stream_end(&view);
        if end <= self.released.len() {
            return None;
        }
        let piece = view[self.released.len()..end].to_string();
        self.released.push_str(&piece);
        Some(piece)
    }
}

/// Byte offset in `view` up to which no later delta can change the text:
/// before the trailing token, and before an unclosed private-key block.
fn stable_stream_end(view: &str) -> usize {
    let mut end = view
        .char_indices()
        .rev()
        .find(|(_, ch)| ch.is_whitespace())
        .map_or(0, |(idx, ch)| idx + ch.len_utf8());
    if let Some(begin) = view.rfind("-----BEGIN")
        && !view[begin..].contains("-----END")
    {
        end = end.min(begin);
    }
    end
...
#[cfg(test)]
mod stream_redactor_tests {
    use super::*;

    fn redactor() -> OutboundStreamRedactor {
        OutboundStreamRedactor::new(
            HashSet::new(),
    ...
    fn stream(chunks: &[&str]) -> String {
        ...
        chunks
            .iter()
            .filter_map(|chunk| redactor.push(chunk))
            .collect()
    }

    const KEY: &str = "sk-ant-api03-abcdefghijklmnopqrstuvwxyz0123456789ABCD";

    #[test]
    fn clean_text_streams_up_to_the_last_token() {
        assert_eq!(
            stream(&["Hello ", "wor", "ld and more"]),
            "Hello world and "
        );
    }

    #[test]
    fn secret_split_across_chunks_is_never_sent_in_clear() {
        let (head, tail) = KEY.split_at(12);
        let sent = stream(&["Your key is ", head, tail, " and that is all. "]);
        assert!(!sent.contains(head), "{sent}");
        assert!(!sent.contains(tail), "{sent}");
        assert!(sent.contains("[REDACTED"), "{sent}");
    }

    #[test]
    fn secret_split_mid_token_after_a_space_is_held_whole() {
        // Every byte boundary: no chunking of the key leaks any prefix of it
        // longer than the fixed `sk-` marker.
        for cut in 4..KEY.len() {
            let (head, tail) = KEY.split_at(cut);
            let sent = stream(&["token ", head, tail, " done "]);
            assert!(!sent.contains(&KEY[..4]), "cut {cut}: {sent}");
        }
    }

    #[test]
    fn private_key_block_is_held_until_it_closes() {
        let mut redactor = redactor();
        ...
        for chunk in [
            ...
            sent.extend(redactor.push(chunk));
        }
        assert!(!sent.contains("MIIEvQIBADANBgkqhkiG9w0BAQEFAASC"), "{sent}");
        assert!(!sent.contains("c2VjcmV0Ym9keWxpbmU="), "{sent}");
    }

    #[test]
    fn think_and_tool_call_tags_never_stream() {
        let sent = stream(&[
            "<think>plan the ",
            "secret route</think>Answer ",
            "<tool_call>{\"name\":\"shell\",",
            "\"arguments\":{}}</tool_call> is here ",
        ]);
        assert!(!sent.contains("think"), "{sent}");
        assert!(!sent.contains("secret route"), "{sent}");
        assert!(!sent.contains("tool_call"), "{sent}");
        assert!(sent.starts_with("Answer "), "{sent}");
    }

    #[test]
    fn final_response_strips_tags_and_redacts() {
        let final_text = sanitize_outbound_response(
            &format!("<think>internal</think>The key is {KEY}"),
            &HashSet::new(),
            &LeakDetectionConfig::default(),
            OutboundContentFormat::Markdown,
        );
        assert!(!final_text.contains("internal"), "{final_text}");
        assert!(!final_text.contains(KEY), "{final_text}");
        assert!(final_text.contains("[REDACTED"), "{final_text}");
    }
}
