...
//! fingerprint recorded in `device_identities.db`. The operator-approved
//! capability ceiling is bound to the pairing code at issue time; enroll
//! inherits that ceiling and never accepts a device-declared one.
...
const MAX_CEILING_CAPS: usize = 16;
...
impl std::fmt::Display for IdentityError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::InvalidCeiling => write!(f, "invalid capability ceiling"),
...
pub fn validate_ceiling(ceiling: &[String]) -> Result<(), IdentityError> {
    if ceiling.len() > MAX_CEILING_CAPS {
        return Err(IdentityError::InvalidCeiling);
    }
    if ceiling
        .iter()
        .any(|cap| cap.is_empty() || cap.len() > MAX_CAP_NAME_BYTES)
    {
        return Err(IdentityError::InvalidCeiling);
    }
    Ok(())
...
impl DeviceIdentityStore {
    ...
    pub fn open(data_dir: &Path, max_entries: usize) -> Result<Self, rusqlite::Error> {
    ...
    pub fn issue_pairing_code(
        ...
        validate_ceiling(&capability_ceiling)?;
        let mut bytes = [0u8; 8];
        SystemRandom::new()
            .fill(&mut bytes)
        ...
        if pending.len() >= self.inner.max_entries {
    ...
    pub fn enroll(
        ...
        let mut pending = self.inner.pairing.lock();
        pending.retain(|_, item| item.expires_at > Instant::now());
        let Some(issued) = pending.get(code) else {
            return Err(IdentityError::InvalidPairingCode);
        };
...
mod tests {
    ...
    fn pairing_binds_fingerprint_and_ceiling() {
        ...
        assert!(store.enroll(&code, keys.public_key_hex()).is_err());
    }
    ...
    fn expired_or_unknown_pairing_code_fails() {
        ...
        let keys = DeviceKeyPair::generate().unwrap();
        assert_eq!(
            store.enroll("missing", keys.public_key_hex()),
            Err(IdentityError::InvalidPairingCode)
        );
    }

    #[test]
    fn malformed_key_does_not_consume_pairing_code() {
        let store = DeviceIdentityStore::memory();
        let keys = DeviceKeyPair::generate().unwrap();
        let code = store
            .issue_pairing_code(vec!["system.notify".into()])
            .unwrap();
        assert_eq!(
            store.enroll(&code, "not-hex"),
            Err(IdentityError::InvalidPublicKey)
        );
        assert_eq!(
            store.enroll(&code, "aa"),
            Err(IdentityError::InvalidPublicKey)
        );
        let identity = store
            .enroll(&code, keys.public_key_hex())
            .expect("code must survive a malformed-key race");
        assert_eq!(identity.capability_ceiling, ["system.notify"]);
        assert_eq!(store.pairing_len(), 0);
    }

    #[test]
    fn pairing_map_and_identity_table_are_bounded() {
        let store = DeviceIdentityStore::memory_with_capacity(1);
        let first = store
            .issue_pairing_code(vec!["system.notify".into()])
            .unwrap();
        assert_eq!(
            store.issue_pairing_code(vec!["system.notify".into()]),
            Err(IdentityError::Capacity)
        );
        let keys = DeviceKeyPair::generate().unwrap();
        store.enroll(&first, keys.public_key_hex()).unwrap();
        let second = store
            .issue_pairing_code(vec!["system.notify".into()])
            .unwrap();
