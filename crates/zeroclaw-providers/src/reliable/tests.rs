...
fn endpoint_from_error_text_preserves_ipv6_host_brackets() {
    ...
    let with_port =
        endpoint_from_error_text("error sending request for url (http://[::1]:8080/v1): failed");
    ...
    ReliableModelProvider::new_with_entries(
        ...
        vec![
            ReliableModelProviderEntry::new(
                "primary",
                "primary.key",
                Box::new(StreamingRecordMock::unsupported(primary_calls)) as Box<dyn ModelProvider>,
            ),
            ReliableModelProviderEntry::new_pinned(
                "fallback",
                "fallback.key",
