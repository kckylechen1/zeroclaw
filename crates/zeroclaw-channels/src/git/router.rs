...
mod tests {
    ...
    fn explicit_entries_override_defaults_either_way() {
        ...
        table.insert(EVT_ISSUE_COMMENT_CREATED.to_string(), route(false));
        // …and opting IN to a non-default.
        table.insert(EVT_WORKFLOW_RUN_FAILED.to_string(), route(true));
        assert_eq!(
            resolve_route(EVT_ISSUE_COMMENT_CREATED, &table),
            RouteAction::Ignore
        );
    ...
    #[test]
    fn validate_routes_flags_typos() {
        let mut table = HashMap::new();
        table.insert("pull_request.create".to_string(), route(true));
    ...
    fn transport_plan_empty_when_everything_opted_out() {
        ...
        for evt in [
            ...
            table.insert(evt.to_string(), route(false));
        }
        let plan = TransportPlan::from_routes(&table);
        assert!(!plan.any());
    }
