...
- `reasoning_effort_passthrough`: send `[runtime] reasoning_effort` to every model on an OpenAI-compatible provider, not only OpenAI reasoning models. Off by default because some backends reject the field; enable it only for a backend known to accept it.
