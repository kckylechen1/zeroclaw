...
| Crate | Role |
|---|---|
| `zeroclaw-runtime` | Agent loop, security policy enforcement, SOP definition loading (the run engine was removed with the run side), cron scheduler, SubAgents, RPC layer for zerocode |
