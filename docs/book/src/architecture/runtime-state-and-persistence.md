...
| Background task supervision | Durable task control plane | `data/control_plane.db` | control-plane handle, coordinator persistence, task producers, and reaper | Owner PID/boot ID identifies prior-boot orphans; heartbeat timeout applies only to producers that emit heartbeats | Rows were written by the detached arms of the retired `spawn_subagent`/`delegate` tools; after the spawn wall no production writer mints new child rows (legacy-row disposition is the control-plane migration wall's scope). `parent_id` is the parent session key so the announce chain can claim what exists. Goal APIs exist, but end-to-end goal execution is not yet wired. |
| Background child results (the retired background `delegate` was the first consumer) | Coordinator child + announce chain | `data/control_plane.db` task row | coordinator actor (query/cancel/list) and parent-turn claim | In-memory query state is process-local; undelivered completions survive restart in the task store until claimed | The workspace `delegate_results/` file store is retired along with the delegate tool. `check_result` reads the live coordinator; leftover JSON files are ignored. |
