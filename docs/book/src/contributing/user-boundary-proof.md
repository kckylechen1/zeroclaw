...
| Background work | A cron job, delegated task, or runtime-spawned subagent starts through its actual lifecycle owner (legacy SOP runs were removed with the run side) and exposes only the status, persistence, cancellation, retry, or restart behavior that path supports. | Drive the changed path through its documented owner and status surface with a deterministic clock or controlled worker; use its configured store only when the claim includes persistence. | The claim depends on wall-clock scheduling, an actual service restart, process supervision, or an external scheduler or worker environment. |
