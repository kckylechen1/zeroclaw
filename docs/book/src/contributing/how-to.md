...
- Reference pages (`docs/book/src/reference/cli.md`, `config.md`) are ignored generated outputs; don't hand-edit or commit them. Run `cargo mdbook refs` to preview changes from the owning CLI/config source.
...
**Contraction PRs** (anything that removes, renames, demotes, sunsets, or deprecates a model-visible route, tool, profile, config key, env override, or code path) must add an entry to `scripts/ci/retired_surfaces.json` in the same PR. The CI gate `scripts/ci/retired_docs_gate.sh` then scans active documentation and fails the PR while any doc still teaches the retired surface as current behavior. Historical mentions that explicitly describe the surface as removed, deprecated, or legacy pass; the registry file documents the entry shape and the discriminator's limits.
