...
**Contraction PRs** (anything that removes, renames, demotes, sunsets, or deprecates a model-visible route, tool, profile, config key, env override, or code path) update or delete the docs that describe it in the same PR. Historical mentions that explicitly describe the surface as removed, deprecated, or legacy are fine.
