...
mod tests {
    ...
    fn ensure_map_key_for_prop_path_leaves_existing_hyphenated_alias_alone() {
        ...
        let err =
            commands::config::ensure_map_key_for_prop_path(&mut config, "cron.bad-alias.name")
    ...
    fn config_init_alias_is_idempotent() {
        let mut config = Config::default();
        commands::config::init_map_alias(&mut config, "risk_profiles.strict")
            .expect("first create");
    ...
    fn config_init_ignores_resource_keyed_sections() {
        let mut config = Config::default();
        assert!(
            commands::config::init_map_alias(
                &mut config,
                "cost.rates.providers.models.openai.gpt-5"
            )
