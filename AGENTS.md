...
| WeChat atomic / non-blocking state persistence | `write_private` still does blocking `std::fs::write`, non-atomic truncate, chmod after write | ✅ carried — atomic tmp+chmod+rename; `save_account_data` is `async` and runs the write on `spawn_blocking` |
