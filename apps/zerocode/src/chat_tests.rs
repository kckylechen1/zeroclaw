...
fn visible_right_tracker_carves_column() {
    let mut t =
        crate::todo_tracker::TodoTracker::new(crate::todo_tracker::TodoLocation::Right, true, true);
...
fn model_provider_picker_overlay_rows_are_hit_testable() {
    let mut s = state();
    s.model_picker = ModelPickerOverlay::ConfiguredProviderStage(crate::widgets::PickerState::new(
...
fn model_picker_overlay_open_states_report_open() {
    let model = ModelPickerOverlay::Model(crate::widgets::PickerState::new(vec!["a".into()], None));
...
async fn acp_init_session_picker_cancel_starts_fresh_session() {
    ...
    let request = next_rpc_request(&mut rx, "fresh session should refresh model identity").await;
    assert_eq!(request["method"], method::CONFIG_LIST);
    respond_ok(&rpc, &request, serde_json::json!([]));
...
async fn acp_init_clears_stale_carried_resume_for_disabled_agent() {
    ...
    let request = next_rpc_request(&mut rx, "fresh fallback should load todotracker config").await;
    assert_eq!(request["method"], method::CONFIG_LIST);
    assert_eq!(request["params"]["prefix"], "todotracker");
    respond_ok(&rpc, &request, serde_json::json!([]));
    ...
    let request = next_rpc_request(&mut rx, "fresh fallback should refresh model identity").await;
    assert_eq!(request["method"], method::CONFIG_LIST);
...
async fn session_picker_double_click_resumes_selected_session() {
    ...
    let request = next_rpc_request(&mut rx, "double-click should resume selected session").await;
    assert_eq!(request["method"], method::SESSION_NEW);
...
fn cached_total_rows_matches_full_line_count() {
    ...
    for i in 0..50 {
        s.push_user_message(
            Some(format!(
                "message number {i} with enough text to wrap across the forty column width budget"
            )),
            Vec::new(),
        );
    }
...
fn session_list_overlay_uses_theme_background_after_clear() {
    ...
    let selected_text_cell = &terminal.backend().buffer()[(overlay_area.x + 4, overlay_area.y + 1)];
    assert_eq!(
...
fn thought_flushed_as_entry_before_tool_call() {
    ...
    assert!(matches!(&s.entries()[0], ChatEntry::AgentThought(t) if t.as_ref() == "plan: run ls"));
...
fn commit_turn_does_not_duplicate_already_flushed_text() {
    ...
    assert!(matches!(&s.entries()[0], ChatEntry::AgentMessage(t) if t.as_ref() == "Before tool."));
    assert!(matches!(&s.entries()[1], ChatEntry::Tool { .. }));
}
...
fn md_code_block_body_is_syntax_highlighted() {
    let _g =
        theme::set_active_for_test(theme::theme_by_name("icy_blue").expect("icy_blue registered"));
